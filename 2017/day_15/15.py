...
def _nxt(start: (int, int), factor: (int, int), cnt: int) -> Generator[tuple, None, None]:
    curA, curB = start
...
def _nxt2(start: (int, int), factor: (int, int), cnt: int) -> Generator[tuple, None, None]:
    curA, curB = start
    factorA, factorB = factor
    for _ in range(cnt):
        while True:
...
def part1(genA: (int, int), genB: (int, int), cnt: int) -> int:
    startA, factorA = genA
    startB, factorB = genB
    return sum([ 1 for a, b in _nxt((startA, startB), (factorA, factorB), cnt) if _lowest16(a) == _lowest16(b) ])


def part2(genA: (int, int), genB: (int, int), cnt: int) -> int:
    startA, factorA = genA
    startB, factorB = genB
...
if __name__ == '__main__':
    with open('input.txt') as f:
        startA, startB = [ int(l.split()[-1]) for l in f.read().splitlines() ]
