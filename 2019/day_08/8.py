...
def part2(values: list, L: int, H: int) -> str:
    ...
    for i in range(0, len(values), layer_size):
        for row, col in filter(lambda x: image[(x[1], x[0])] == 2, product(range(H), range(L))):
            layer = values[i:i + layer_size]
            image[(col, row)] = layer[row * L + col]
    return '\n'.join([ ''.join([ _draw(image[k]) for k in image.keys() if k[1] == i ]) for i in range(H) ])
